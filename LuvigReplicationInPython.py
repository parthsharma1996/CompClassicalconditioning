from __future__ import division
# -*- coding: utf-8 -*-
"""
Created on Fri Jun  3 16:16:50 2016

@author: parth
"""

# -*- coding: utf-8 -*-
"""
Created on Thu Jun  2 00:46:31 2016

@author: parth
"""

# -*- coding: utf-8 -*-


import numpy as np
import matplotlib.pyplot as plt
import math 

fig = plt.figure()
ax1 = fig.add_subplot(1,1,1)
'''Constants '''

alpha=0.1
gamma=0.98
decay=0.985
sigma=0.08
Lambda=0.95

'''Defining matrices'''
m=50
n=500
x=np.zeros((2*m+1,n+1))
elig=np.zeros(2*m+1)
#w=np.zeros(2*m+1)
delW=np.zeros(2*m+1)
delta=np.zeros(n+1)
r=np.zeros(n+1)
v=np.zeros(n+1)
y_array=[]

'''Functions'''

def gaussian(x, mu, sig):
    return np.exp(-np.power(x - mu, 2.) / (2 * np.power(sig, 2.)))/np.sqrt(2*np.pi)

def x_i_t(i,t,shift):
    '''gives the value of x_i_t'''
    if(t-shift<0):
        y=0
    else:
        y=math.pow(decay,t-shift)
        #print "y",y
    y_array.append(y)
    x_val=gaussian(y,i/m, sigma)*y

    return x_val
    


def plotDelta():
    
    plt.plot(delta)
    plt.show()
    
    
def plotValue():
    
    plt.plot(v)
    plt.show()
    
def StimAndRAssign(rewardMag,rewardT,stimulusT):
    r[rewardT]=rewardMag
    for i in range(0,m+1):  # let stimulus be at t=20 and reward at t=80
        for t in range(0,n+1):   
            x[i][t]=x_i_t(i,t,stimulusT)
            x[i+m][t]=rewardMag*x_i_t(i,t,rewardT)
#    
def runTrial(w):
    elig=np.zeros(2*m+1)
    oldValue=0

    
    for time in range(0,n):
        v[time]=max(0,np.dot(w.T,x[:,time]))
            
        delta[time]=r[time]+gamma*v[time]-oldValue
#            print "delta",delta
        delW=alpha*delta[time]*elig
#            print "delW",delW
        w=w+delW
#            print "w", w
        elig=gamma*Lambda*elig+x[:,time]
        oldValue=max(0,np.dot(w,x[:,time]))
    return w
        
    
def simpleAcq(numTrials,rewardMag,rewardT,stimulusT):
    w=np.zeros(2*m+1)

    StimAndRAssign(rewardMag,rewardT,stimulusT);
   
    for trials in range(0,numTrials):
        w=runTrial(w)
    return w            

def rewOmmision():
    
    simpleAcq(999,20,5);
    StimAndRAssign(0,20,5);
    runTrial()
    plotDelta()
    plotValue()
    
    


def main():
    rewOmmision()
    

main()